...
def main(argv):
    ...
    label_PF = tk.Label(master=frame_PF, text=f"PF (13)")
    slider_PM = tk.Scale(master=frame_PM,from_=0, to=180, command=move_PM)
    label_PM = tk.Label(master=frame_PM, text=f"PM (10)")
    slider_PR = tk.Scale(master=frame_PR,from_=0, to=180, command=move_PR)
    label_PR = tk.Label(master=frame_PR, text=f"PR (15)")
    ...
    label_SF = tk.Label(master=frame_SF, text=f"SF (2)")
    slider_SM = tk.Scale(master=frame_SM,from_=0, to=180, command=move_SM)
    label_SM = tk.Label(master=frame_SM, text=f"SM (5)")
    slider_SR = tk.Scale(master=frame_SR,from_=0, to=180, command=move_SR)
    label_SR = tk.Label(master=frame_SR, text=f"SR (11)")
